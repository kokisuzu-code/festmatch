...
export default function ClaimApplicationForm({ token }: { token: string }) {
  ...
  return <div className={styles.form}><p className={styles.notice}>ログイン中のメールアドレスと応募時のメールアドレスが一致する場合、ベンダープロフィールと正式応募を作成します。</p><button className={styles.submit} disabled={sending} onClick={claim}><span>{sending ? "確認中…" : "応募を完了する"}</span>{!sending && <span aria-hidden="true">→</span>}</button>{message && <p className={styles.message} role="alert">{message}</p>}</div>
}
