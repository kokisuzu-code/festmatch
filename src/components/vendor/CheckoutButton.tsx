...
export default function CheckoutButton({ applicationId }: { applicationId: string }) {
  ...
  return <div className="action-box"><button className="button button-primary" disabled={loading} onClick={checkout}>{loading ? "準備中" : "出店料を支払う"}</button>{message && <p>{message}</p>}</div>
}
