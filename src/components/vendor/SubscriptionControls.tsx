...
export default function SubscriptionControls() {
  ...
  return <div className="subscription-controls"><div className="plan-button-grid">{(["light", "standard", "pro"] as const).map((tier) => <button key={tier} className="button button-secondary" disabled={Boolean(loading)} aria-busy={loading === tier} onClick={() => start(tier)}>{loading === tier ? "決済ページを準備中…" : `${tier[0].toUpperCase()}${tier.slice(1)} を選ぶ`}</button>)}</div><button className="text-button" disabled={Boolean(loading)} aria-busy={loading === "portal"} onClick={portal}>{loading === "portal" ? "管理画面を準備中…" : "Stripeのプラン管理を開く"}</button>{message && <p>{message}</p>}</div>
}
