...
export default function ApplyButton({ eventId, closed, disabledReason }: { eventId: string; closed: boolean; disabledReason?: string }) {
  ...
  return <div className="apply-box"><textarea value={message} onChange={(event) => setMessage(event.target.value)} placeholder="主催者へのメッセージ（任意）" rows={3} /><button className="button button-primary" disabled={sending} aria-busy={sending} onClick={apply}>{sending ? "送信中…" : "応募する"}</button><small>無料プランを含め、すべてのベンダーが応募できます。</small>{result && <p>{result}</p>}</div>
}
