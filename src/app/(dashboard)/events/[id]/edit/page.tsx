...
export default function EditEventPage() {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      ...
      setDates(event.dates ?? (event.date ? [event.date] : []))
      setStartTime(event.start_time ?? '10:00')
      setEndTime(event.end_time ?? '17:00')
      setLocation(event.location ?? '')
      setPrefecture(event.prefecture ?? '')
      setExpectedVisitors(event.expected_visitors != null ? String(event.expected_visitors) : '1000')
      setTotalSlots(event.total_slots != null ? String(event.total_slots) : '10')
      setFee(event.fee != null ? String(event.fee) : '10000')
      setApplyDeadline(event.apply_deadline ?? '')
      setDescription(event.description ?? '')
      setHasPower(event.has_power ?? false)
      setHasWater(event.has_water ?? false)
      setHasParking(event.has_parking ?? false)
      setHasDrainage(event.has_drainage ?? false)
      setGeneratorOk(event.generator_ok ?? false)
      setIsIndoor(event.is_indoor ?? false)
      setVendorType(event.vendor_type ?? 'kitchen_car_only')
      setGroundType(event.ground_type ?? 'asphalt')
      setHasWasteDisposal(event.has_waste_disposal ?? false)
  ...
  const handleSubmit = async (status: 'draft' | 'published') => {
    ...
    // ジャンル枠: 既存を全削除してから再挿入
    await supabase.from('event_genre_slots').delete().eq('event_id', eventId)
    const validSlots = genreSlots.filter(s => s.genre.trim() !== '')
    if (validSlots.length > 0) {
      const { error: slotError } = await supabase.from('event_genre_slots').insert(
        validSlots.map(s => ({ event_id: eventId, genre: s.genre, max_count: s.max_count }))
      )
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            {title && (
              <>
                <Link href={`/events/${eventId}`} className="text-xs text-gray-400 hover:text-gray-600 max-w-[180px] truncate">{title}</Link>
                <span className="text-gray-300 text-xs">/</span>
              </>
            )}
            <span className="text-xs text-gray-700 font-medium">編集</span>
          </div>
        </header>

        <main className="flex-1 overflow-auto px-6 py-8">
          <div className="max-w-2xl mx-auto">
            <h1 className="text-2xl font-semibold text-gray-900 mb-8">イベントを編集</h1>
            ...
            <div className="space-y-8">
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {dates.map(d => (
                        <span key={d} className="flex items-center gap-1.5 text-sm bg-green-50 text-green-700 border border-green-200 px-3 py-1 rounded-full">
                          {d}
                          <button type="button" onClick={() => setDates(p => p.filter(x => x !== d))} className="text-green-400 hover:text-red-500">x</button>
                        </span>
                      ))}
                    </div>
                  )}
              ...
              <section className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
                ...
                {genreSlots.map((slot, i) => (
                  <div key={i} className="flex gap-3 items-center">
                    ...
                    {genreSlots.length > 1 && (
                      <button type="button" onClick={() => removeGenreSlot(i)} className="text-gray-400 hover:text-red-500">x</button>
                    )}
                  </div>
                ))}
              </section>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {customFacilityTags.map(tag => (
                        <span key={tag} className="flex items-center gap-1.5 text-xs bg-gray-100 text-gray-700 px-2.5 py-1 rounded-full">
                          {tag}
                          <button type="button" onClick={() => removeTag(tag, customFacilityTags, setCustomFacilityTags)} className="text-gray-400 hover:text-red-500">x</button>
                        </span>
                      ))}
                    </div>
                  )}
                  ...
                  {customEnvTags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {customEnvTags.map(tag => (
                        <span key={tag} className="flex items-center gap-1.5 text-xs bg-gray-100 text-gray-700 px-2.5 py-1 rounded-full">
                          {tag}
                          <button type="button" onClick={() => removeTag(tag, customEnvTags, setCustomEnvTags)} className="text-gray-400 hover:text-red-500">x</button>
                        </span>
                      ))}
                    </div>
                  )}
              ...
              <div className="flex gap-3">
                <Link href={`/events/${eventId}`}
                  className="flex-1 border border-gray-300 rounded-lg py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors text-center">
                  キャンセル
                </Link>
